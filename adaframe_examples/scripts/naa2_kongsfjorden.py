...
class ChlaTracker:
    def __init__(self):
        ...
        self.max_depth                  = rospy.get_param("/" + self.node_name + "/max_depth", 40.0)
        ...
        self.speed                      = rospy.get_param("/" + self.node_name + "/desired_speed", 1.3)
        ...
        z_extent = rospy.get_param("/" + self.node_name + "/z_extent", 40)
    ...
    def IridiumMsgRxCB(self,msg):
        ...
        dd = bytes.fromhex(msg.data.data)
        bb = []
        ...
        mno = struct.unpack("B",dd[0])
        if mno is 99:
            time_sent = struct.unpack("f",dd[1:5])[9]
            ...
            for i in range(lenth):
                ...
                c.append(struct.unpack("B",dd[22+i+lenth:23+i+lenth])[0]/10.0)
                d.append(struct.unpack("B",dd[22+i+2*lenth:23+i+2*lenth])[0]/10.0)
    ...
    def sendAndUpdate(self):
        if self.auv_handler.vehicle_state == "executing" or self.auv_handler.vehicle_state == "waiting":
            ...
            lat,lon = self.grid2wp(self.vehicle_pos[0]+100.0,self.vehicle_pos[1]+50.0)
            self.auv_handler.setWaypoint(lat,lon, z = 0.01, speed = self.speed)
            ...
            while self.auv_handler.getProximity() == "far":
                ...
                print(rospy.get_time() - ptime)
                print(self.auv_handler.getProximity())
            ...
            if self.ada_state == "adapting":
                ...
                while rospy.get_time() - self.last_surface_time <= self.seconds_on_surface and self.iridium_status == "busy":
                    self.auv_handler.spin()
                    print(rospy.get_time() - self.last_surface_time)
                    print(self.iridium_status)
                    self.iridium_status = "ready"
        ...
        for path in pp:
            ...
            if path[0] > 0.0 and path[0] < self.grid_extent[0]:
                if path[1] > 0.0 and path[1] < self.grid_extent[1]:
                    ...
                    print("Avoidance score:")
                    print(avoidance_score)
                    values = []
                    ss = 0
    ...
    def updateWP(self):
        ...
        if self.ada_state == "init":
            ...
            if self.auv_handler.getProximity() == "z_near" and self.ref_init:
