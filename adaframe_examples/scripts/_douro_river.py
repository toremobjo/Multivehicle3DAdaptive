...
class DouroTracker:
    def __init__(self):
        ...
        self.phone_number = rospy.get_param("/" + self.node_name + "/phone_number")
    ...
    def updateWP(self):
        ...
        if self.ada_state == "init":
            ...
            if len(self.depth_estimates) is not 0:
                ...
                X = np.c_[ np.ones(len(x)),x,y,np.square(x),np.square(y),np.multiply(x,y)]
                ...
                mu = b[0] + b[1] * x + b[2] * y + b[3]*np.square(x) + b[4]* np.square(y) + b[5]*np.multiply(x,y)
                ...
                pred = b[0] + b[1] * x_star + b[2] * y_star + b[3]*np.square(x_star) + b[4]* np.square(y_star) + b[5]*np.multiply(x_star,y_star)
                pred = np.array(pred)
    ...
    def run(self):
        while not rospy.is_shutdown():
            if self.init:
                ...
                if self.auv_handler.getProximity() == "z_near" and self.ada_state == "adapting":
                    self.updateWP()
