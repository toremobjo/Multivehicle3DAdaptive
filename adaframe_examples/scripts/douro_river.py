...
class DouroTracker:
    def __init__(self):
        ...
        self.seconds_between_surface = 120
        self.seconds_on_surface = 60
        ...
        self.river_mouth_angle_north = 300 * np.pi/180.0
        ...
        self.seconds_between_surface = rospy.get_param("/" + self.node_name + "/popup_interval", 10.0)
        ...
        self.mission_length = rospy.get_param("/" + self.node_name + "/mission_length", 360000)
        ...
        self.seconds_on_surface = rospy.get_param("/" + self.node_name + "/seconds_on_surface", 60)
        self.river_salinity = rospy.get_param("/" + self.node_name + "/river_salinity", 28)
        self.ocean_salinity = rospy.get_param("/" + self.node_name + "/ocean_salinity", 33)
        ...
        self.log_name = "duoro_river" + str(rospy.get_time())
        self.log_directory = "/home/tore/software/eumr/adaframe_ws/logs/douro_river/"
        ...
        ############ GP values ##############
        self.origin = np.array([41.14028883,-8.68411685])#np.array([41.12696217,-8.7107383])
        ...
        self.auv_handler.setWaypoint(self.prior_start[0]*np.pi/180.0,self.prior_start[1]*np.pi/180.0, speed = self.speed)
        self.rate.sleep()
    ...
    # Covariance function
    def getcov_old(self,d_sites, p_sites, par):
        sig2 = par[0] ** 2
        crange = par[1]
    ...
    def getcov(self, d_sites, p_sites, par):
        ...
        h = scip.distance.cdist(d_sites, p_sites, 'euclidean')
        for i in np.nditer(h, op_flags=['readwrite']):  # Modifying array values
            i[...] = sig2 * np.exp(-0.5 * ((i / crange) ** 2))
        return np.transpose(h)
    ...
    def generatePaths(self):
        ...
        for path in sorted_paths:
            ...
            for i in range(self.gp_resolution-1):
                len += np.linalg.norm([self.xgrid[path[i+1]]-self.xgrid[path[i]],self.ygrid[path[i+1]],self.ygrid[path[i]]])
            lens.append(len)
    ...
    def measureSgrad(self):
        ...
        if len(depths) > 20:
            ...
            if max(salinities) > 33 and min(salinities) < 28:
                ...
                for i in range(len(predictions)-1):
                    if predictions_dx[i+1] > 30.5 and predictions_dx[i] < 30.5:
                        depth = offsets[i]
                    elif predictions_dx[i+1] < 30.5 and predictions_dx[i] > 30.5:
                        depth = offsets[i]
                    #predictions_dx.append(abs((predictions[i+1] - predictions[i])/(offsets[i+1]-offsets[i])))
                #index = np.argmax(predictions_dx)
                if depth > 0.0:
                    gradmax.append(depth)
                    print("depth of river: ", depth)
            elif min(salinities) > 33:
                gradmax.append(0.0)
            elif max(salinities) < self.river_salinity:
            ...
            if self.iteration%10 and self.ada_state == "adapting":
                df = pd.DataFrame({"depths":self.depth_estimates})
                df.to_csv(self.log_directory+self.log_name+".csv")
                print("saved log")
    ...
    def updateWP(self):
        ...
        if rospy.get_time() - self.last_surface_time > self.seconds_between_surface:
            if self.ada_state == "adapting" or self.ada_state == "prior":
                self.auv_handler.PopUp(sms=True, iridium=True, popup_duration = self.seconds_on_surface, phone_number=self.phone_number, iridium_dest = self.iridium_destination)#self.ada_state = "surfacing"
                self.last_surface_time = rospy.get_time()

        if self.ada_state == "init":
            print(self.iteration)
            self.auv_handler.setWaypoint(self.prior_wps[self.iteration][0]*np.pi/180.0,self.prior_wps[self.iteration][1]*np.pi/180.0,z=self.prior_wps[self.iteration][2], speed = self.speed)
            ...
            if self.iteration  < len(self.prior_wps):
                ...
                self.auv_handler.setWaypoint(self.prior_end[0]*np.pi/180.0,self.prior_end[1]*np.pi/180.0, z = 0.0, speed = self.speed)
            ...
            if len(self.depth_estimates) is not 0:
                ...
                d_depths = depths - mu
                t = rospy.get_time()
                k_bs = self.getcov(data_sites, pred_sites, cov_param)
                k_sb = self.getcov(pred_sites, data_sites, cov_param)
                k_ss = self.getcov(pred_sites, pred_sites, cov_param)
                # Weights for similarity of measurements
                k_bb = self.getcov(data_sites, data_sites, cov_param)
                ...
                tau = 0.75#np.sqrt(np.var(depths[-20:-1]))
                #if tau < 0.1:
                #    tau = 0.1
                ...
                pred_field = np.transpose(k_sb) @ np.linalg.inv(k_bb) @ d_depths + pred
                resulting_cov = k_ss - np.transpose(k_sb) @ np.linalg.inv(k_bb) @ np.transpose(k_bs)

                scores = []
                self.k_l = -0.001/np.sqrt(2)
                self.k_var = 1.5#3
                self.k_depth = 4 #3
                self.k_diff =  1.5#1.6#0.8
                depth_intercept = 2
                nugget = 0.2
                c = 0
                for path in self.paths:
                    score = 100.0# self.lens[c]*self.k_l + 1000.0
                    #print("length score: ", self.lens[c]*self.k_l)
                    c+=1
                    cc = 0
                    c_score = 0.0
                    d_score = 0.0
                    diff_score = 0.0
                    for i in path:
                        c_score += self.k_var*resulting_cov[i][i]#*(self.gp_resolution-cc)/self.gp_resolution
                        d_score += self.k_depth*pred_field[i]/self.max_depth#(1/(1 + np.exp(-pred_field[i]-self.lowpass_depth)))#*(cc+1)*0.5#*(self.gp_resolution-cc)/self.gp_resolution
                        diff_score += self.k_diff*np.abs(pred_field[i]-self.lowpass_depth)
                        ...
                        if self.is_illigal(self.grid2wp(wpx,wpy)):
                            score -= 1000000.0
                    #print("cov score: ", c_score)
                    #print("depth score: ", d_score)
                    score += c_score
                    score += d_score
                    score += diff_score
                ...
                print(self.iteration)
                self.curr_wp = wp
                self.auv_handler.setWaypoint(wp[0],wp[1], z = self.desired_z, speed = self.speed)
