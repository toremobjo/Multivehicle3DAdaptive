#!/usr/bin/env python3
# license removed for brevity Adaptive sampling group of NTNU Tore Mo-Bjund 2022 contact: 
# tore.mo-bjorkelund@ntnu.no
...
class ChlaTracker:
    def __init__(self):
        ...
        self.max_depth                  = rospy.get_param("/" + self.node_name + "/max_depth", 50.0)
        ...
        self.origin_lat                 = rospy.get_param("/" + self.node_name + "/origin_lat",78.9363)
        self.origin_lon                 = rospy.get_param("/" + self.node_name + "/origin_lon", 11.9687)
        self.speed                      = rospy.get_param("/" + self.node_name + "/desired_speed", 1.4)
        ...
        x_extent = rospy.get_param("/" + self.node_name + "/x_extent", 1500)
        y_extent = rospy.get_param("/" + self.node_name + "/y_extent", 1500)
        z_extent = rospy.get_param("/" + self.node_name + "/z_extent", 50)
        ...
        x_lscale = rospy.get_param("/" + self.node_name + "/x_lscale", 300)
        y_lscale = rospy.get_param("/" + self.node_name + "/y_lscale", 300)
        z_lscale = rospy.get_param("/" + self.node_name + "/z_lscale", 3)
        self.lscales = [x_lscale,y_lscale,z_lscale]

        self.sigma2 = rospy.get_param("/" + self.node_name + "/sigma2", 4.0)
        self.nugget = rospy.get_param("/" + self.node_name + "/nugget", 0.4)
        self.time_sat = rospy.get_param("/" + self.node_name + "/time_sat", 10000)
        self.avoidance_coeff = rospy.get_param("/" + self.node_name + "/avoidance_coeff", 300)
        ...
        for i in range(0,int(self.dives*2)+1):
            ...
            self.prior_wps.append([lat,lon,(i%2)*self.max_depth+0.01])
    ...
    def IridiumMsgRxCB(self,msg):
        ...
        mno = struct.unpack("B",dd[0].to_bytes(1,byteorder="big"))[0]
        print(mno)
        print(dd)
        if mno is 99:
            ...
            for i in range(lenth):
                ...
                c.append(struct.unpack("H",dd[22+2*i+lenth:24+2*i+lenth])[0])
    ...
    def sendAndUpdate(self):
        if self.auv_handler.vehicle_state == "executing" or self.auv_handler.vehicle_state == "waiting":
            ...
            self.auv_handler.setWaypoint(lat,lon, z = 0.01, speed = self.speed)
            ...
            pup = False
            while self.auv_handler.getProximity() == "far":
                self.auv_handler.spin()

                if rospy.get_time() - self.last_surface_time > self.popup_timeout:
                    rospy.signal_shutdown("too long submerged")

                if rospy.get_time() - ptime > 30 and not pup:
                    pup = True
                    lat,lon = self.grid2wp(self.vehicle_pos[0]+100,self.vehicle_pos[1]+50)
                    self.auv_handler.setWaypoint(lat,lon, z = 0.01, speed = self.speed)
            ...
            if self.ada_state == "adapting":
                ...
                while not self.waiting_for_iridium_data or rospy.get_time() - self.last_surface_time <= self.seconds_on_surface:
                    self.auv_handler.spin()
                self.suspended = False
                self.auv_handler.setWaypoint(lat,lon,z=d, speed = self.speed)
    ...
    def updateWP(self):
        ...
        rospy.loginfo("ada state: %s", self.ada_state)
        rospy.loginfo("vstate: %s" , self.auv_handler.getState())
        rospy.loginfo("Proxx: %s", self.auv_handler.getProximity())
        ...
        if self.ada_state == "init":
            ...
            #if not self.auv_handler.getProximity() == "z_near":
            ...
            if self.init:
                ...
                if state == "waiting" and (rospy.get_time()-self.last_wp_update>self.wp_update_timeout):
                    if not self.suspended:
                        ...
                        self.updateWP()
                '''
                elif self.auv_handler.getProximity() == "xyz_near" and (rospy.get_time()-self.last_wp_update>self.wp_update_timeout):
                    if not self.suspended:
                        self.last_wp_update = rospy.get_time()
                        self.updateWP()

                elif self.auv_handler.getProximity() == "xy_near" and (rospy.get_time()-self.last_wp_update>self.wp_update_timeout):
                    if not self.suspended:
                        self.last_wp_update = rospy.get_time()
                        self.updateWP()

                elif self.auv_handler.getProximity() == "z_near" and (rospy.get_time()-self.last_wp_update>self.wp_update_timeout):
                    if not self.suspended:
                        self.last_wp_update = rospy.get_time()
                        self.updateWP()
                        self.auv_handler.spin()
